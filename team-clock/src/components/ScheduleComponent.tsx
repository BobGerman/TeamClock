...
class ScheduleComponent extends React.Component<IScheduleComponentProps, IScheduleComponentState> {
  ...
  showSlides(n: number) {
    var i;
    var slides = document.getElementsByClassName("timeSlides") as any;
  ...
  _getItemsPerSlide() {
    let containerWidth;
    let slideElement = document.getElementsByClassName("timeSlotsContainer") as any;
  ...
  _renderSlides(date: Date) {
    let itemsPerSlide = this._getItemsPerSlide();

    //Get the next 12 hours for the current user to check schedules
    let hours: string[] = [];// this.props.clockService.getMeetingHours(date, 'H', this.props.timeZoneObj.timeZone, 24);
    //ddd. ha
    let timeSlots: any[] = []; // TODO: Fix any

    //Create an array the hours in the current users timezone
    hours.map((m, index) => {

      //Loop through participants
      let hoursLIs: any[] = []; // TODO: Fix any
      this.props.participants.map((participant, index) => {
        ...
        meetingDate.setHours(parseInt(m));
        // Was this but it won't compile because convertTimeZone has only 2 args:
        // hour = this.props.clockService.convertTimeZone(meetingDate, 'ddd. ha', participant.timeZone);
        let hour = this.props.clockService.convertTimeZone(meetingDate, 'ddd. ha');
  ...
  _renderParticipants() {
    //Loop through participants
    let participantLIs:any[] = []; // TODO: Fix any
    ...
    this.props.participants.map((participant, index) => {
      ...
      // TODO: Fix any
      let participantLI: any = React.createElement('li', { className: "peopleList", onClick: () => this._removeParticipant(participant), key: "li-" + participant + "-" + index }, span);
